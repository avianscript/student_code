...
document.addEventListener('DOMContentLoaded', evt => {
  ...
  desc.addEventListener( 'click', evt => {
    toggleDescriptionEdit(evt.target);
  });

  const txtBox = document.getElementById('inputDesc');
  txtBox.addEventListener('keyup', (evt) => {
    // console.log('keyup: ' + evt.key);
    if (evt.key === 'Enter') {
      exitDescriptionEdit(evt.target, true);
    } else if (evt.key === 'Escape') {
      exitDescriptionEdit(evt.target, false);
    }
  })

  txtBox.addEventListener('mouseleave', (evt) => {
    exitDescriptionEdit(evt.target, false);
  })

  // txtBox.addEventListener('change', (evt) => {
  //   evt.stopPropagation();
  //   console.log('CHANGED!')
  // });
  ...
  const addHideReviewBtn = document.getElementById('btnToggleForm');
  addHideReviewBtn.addEventListener('click', (evt) => {
    showHideForm();
  });

  // const nameField = document.getElementById('name');
  // nameField.addEventListener('focus', () => {
  //   console.log('name got focus');
  // });
  ...
  main.addEventListener('change', evt => {
    console.log('main got change event too!');
  });

  const saveBtn = document.getElementById('btnSaveReview');
  saveBtn.addEventListener('click', evt => {
    // const saveBtn = document.addEventListener('click', evt => {
