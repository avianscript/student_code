<template>
  <div class="main">
    ...
    <div class="well-display">
      <div class="well" v-on:click="setFilterVal(0)">
        <span class="amount">{{ averageRating }}</span>
        Average Rating
      </div>

      <!-- <div class="well" v-on:click="setFilterVal(1)">
        <span class="amount">{{ numberOfOneStarReviews }}</span>
        1 Star Review{{ numberOfOneStarReviews === 1 ? "" : "s" }}
      </div>

      <div class="well" v-on:click="setFilterVal(2)">
        <span class="amount">{{ numberOfTwoStarReviews }}</span>
        2 Star Review{{ numberOfTwoStarReviews === 1 ? "" : "s" }}
      </div>

      <div class="well" v-on:click="setFilterVal(3)">
        <span class="amount">{{ numberOfThreeStarReviews }}</span>
        3 Star Review{{ numberOfThreeStarReviews === 1 ? "" : "s" }}
      </div>

      <div class="well" v-on:click="setFilterVal(4)">
        <span class="amount">{{ numberOfFourStarReviews }}</span>
        4 Star Review{{ numberOfFourStarReviews === 1 ? "" : "s" }}
      </div>
      <div class="well" v-on:click="setFilterVal(5)">
        <span class="amount">{{ numberOfFiveStarReviews }}</span>
        5 Star Review{{ numberOfFiveStarReviews === 1 ? "" : "s" }}
      </div> -->

      <div class="well" v-for="n in 5" v-bind:key="n"
      v-on:click="setFilterVal(n)">
    ...
    <!-- <a href='' v-on:click.prevent="showForm = true">{{ showHideFromText }} Form</a> -->

    <!-- <a href='' v-on:click.prevent="showForm = true">{{ showForm ? 'Hide' : 'Show' }} Form</a> -->
  ...
  computed: {
    ...
    filteredReviews() {
      return this.reviews.filter(
        (review) => this.filterVal === 0 || review.rating === this.filterVal
      );
    },
  },
  methods: {
    ...
    numberOfReviews(stars) {
      const reviewSet = this.reviews.filter((review) => review.rating == stars);
      return reviewSet.length;
    },
  },
};
</script>
