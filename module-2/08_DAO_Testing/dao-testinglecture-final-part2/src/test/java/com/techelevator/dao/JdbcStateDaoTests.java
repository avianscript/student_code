...
public class JdbcStateDaoTests extends BaseDaoTests {
    ...
    public void createState() {
        State newState = new State("JM", "Jimstate");
        State myState = sut.createState(newState);

//        State myState = sut.getState("JM");


        Assert.assertNotNull(myState);
//
//        Assert.assertEquals("JM", myState.getStateAbbreviation());
//        Assert.assertEquals("Jimstate", myState.getStateName());

        Assert.assertEquals(newState, myState);
    }
